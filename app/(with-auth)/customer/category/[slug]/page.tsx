...
export default async function AuthCategoryDetailPage({
  params,
}: {
  ...
  const [category, products] = await Promise.all([
    catSvc.getCategoryDetail(slug).catch(() => null),
    catSvc.getProductsByCategory(slug),
  ]);

  const categoryProducts = products.filter((product: IProduct) =>
    product.category.some((cat: ICategory) => cat.slug === slug)
  );
